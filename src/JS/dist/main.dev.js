...
function onLoad() {
  for (var _i = 1; _i < 120; _i++) {
    backgroundEffectList[_i] = new Image();
    onloadLock--;
    backgroundEffectList[_i].src = 'images/Source/source (' + _i + ').png';

    backgroundEffectList[_i].addEventListener('load', function (e) {
      onloadLock++;
    });
  }
}
...
function getRandomImages(directory, count) {
  ...
  for (var _i2 = 0; _i2 < count; _i2++) {
    var image = getRandomFile(ids);
    og_images.push(directory + '/' + image + '.png');
    var prefix = image.substring(0, 2); // retrieve first two characters of the string
...
function getRandomFile() {
  var repetitionArrayIn = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : [];
  var id = 20;
  ...
  var file = id + '_' + gender + '_' + race_is_element_id_is_pos[id] + '_' + getRandomInt(2) + '_' + getRandomInt(9);
...
function startGameButton() {
  ...
  var slider = document.getElementById("doppelgangerSlider").querySelector(".slider");
  slider.value = 1;
...
function calcGreyScale() {
  ...
  for (j = 0; j < originalImageData.height; j++) {
    for (i = 0; i < originalImageData.width; i++) {
      ...
      var average = (red + green + blue) / 3; //originalImageData.data[index]=average;
  ...
  greyCanvas = document.createElement('canvas'), ctx = greyCanvas.getContext('2d');
...
function startEdit() {
  ...
  var dsize = new cv.Size(600, 600);
  faceClassifier = new cv.CascadeClassifier();
  faceClassifier.load('haarcascade_frontalface_default.xml');
  faceVect = new cv.RectVector();
  faceClassifier.detectMultiScale(mat, faceVect);
  face = new cv.Rect(faceVect.get(0).x, faceVect.get(0).y, faceVect.get(0).width, faceVect.get(0).height); //cv.resize(mat, resizeMat, dsize, 0, 0, cv.INTER_AREA);
  //cv.imshow("canvas", mat);

  ctx.drawImage(chosenPlayer, 0, 0, 600, 600);
  xScale = 600 / mat.size().width;
  yScale = 600 / mat.size().height; // draw the noise layer:

  var blurNoiseSizeWidth = face.width / 40;
  var blurNoiseSizeHeight = face.height / 40;

  for (var _i3 = 0; _i3 < 41; _i3++) {
    for (var _j = 0; _j < 41; _j++) {
      ctxBlurLayer.fillStyle = 'rgb(' + Math.floor(Math.random() * 256) + ',' + Math.floor(Math.random() * 256) + ',' + Math.floor(Math.random() * 256) + ')';
      ctxBlurLayer.fillRect((face.x + _i3 * blurNoiseSizeHeight) * xScale, (face.y + _j * blurNoiseSizeWidth) * yScale, blurNoiseSizeHeight * xScale, blurNoiseSizeWidth * yScale);
    }
  } // Draw contour
  ...
  for (var _i5 = 0; _i5 < contours.size(); ++_i5) {
    var color = new cv.Scalar(Math.round(Math.random() * 255), Math.round(Math.random() * 255), Math.round(Math.random() * 255));
    cv.drawContours(contour, contours, _i5, color, 1, 8, hierarchy, -1);
  }
  ...
  ctxFullColorLayer.fillStyle = 'rgba(' + Math.floor(Math.random() * 256) + ',' + Math.floor(Math.random() * 256) + ',' + Math.floor(Math.random() * 256) + ',' + 0.2 + ')';
...
function turnOnEffect() {
  for (var _i6 = 0, _effectList = effectList; _i6 < _effectList.length; _i6++) {
    var effect = _effectList[_i6];
    toggelEffect(effect);
  }
}
...
function getDoppelgangerImages(directory, value) {
  ...
  var chosenInfo = chosenPlayer.src;
  var chosenId = chosenInfo.substring(chosenInfo.length - 14, chosenInfo.length - 12);
  ...
  var imagesToReturn = [];
  var race = chosenInfo.substring(chosenInfo.length - 9, chosenInfo.length - 8);
  var background = chosenInfo.substring(chosenInfo.length - 7, chosenInfo.length - 6);
  var costume = chosenInfo.substring(chosenInfo.length - 5, chosenInfo.length - 4);
  var ids = [];
  var idsToSkip = [];
  var file = "";
  ...
  for (var _i7 = genderOffset; _i7 < 12 + genderOffset; _i7++) {
    if (_i7.toString() === chosenId || idsToSkip.includes(_i7.toString())) {
      continue;
    }

    if (value == 2) {
      do {
        file = _i7 + '_' + gender + '_' + race_is_element_id_is_pos[_i7] + '_' + getRandomInt(2) + '_' + getRandomInt(9);
      } while (!fileExists(file));
    }

    if (value == 3) {
      do {
        file = _i7 + '_' + gender + '_' + race_is_element_id_is_pos[_i7] + '_' + background + '_' + getRandomInt(9);
      } while (!fileExists(file));
    }

    if (value == 4) {
      do {
        file = _i7 + '_' + gender + '_' + race_is_element_id_is_pos[_i7] + '_' + background + '_' + costume;
...
function finishEdit() {
  ...
  var agentChoicesLocs = finalAgentChoices;
